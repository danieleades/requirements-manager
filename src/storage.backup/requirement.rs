...
impl Requirement {
    ...
    fn from_markdown<R: BufRead>(reader: &mut R, hrid: String) -> Result<Self, LoadError> {
    ...
    fn from_markdown_str(s: &str, hrid: String) -> Result<Self, serde_yaml::Error> {
        ...
        Self::from_markdown(&mut reader, hrid).map_err(|e| {
            match e {
                ...
                LoadError::Io(_) => unreachable!(),
                LoadError::Yaml(error) => error,
            }
        })
    }
    ...
    pub fn from_file(path: &Path, hrid: String) -> Result<Self, LoadError> {
        let filename = path.join(&hrid);
        let file = File::open(filename).map_err(LoadError::Io)?;
...
impl domain::Requirement {
    /// Load a requirement from file.
    /// 
    /// The `path` is the path to the directory containing the requirement,
    /// the filename is determined by the `hrid`.
    fn load(path: &Path, hrid: String) -> Result<Self, LoadError> {
        let requirement = Requirement::from_file(path, hrid)?.into();
        Ok(requirement)
    }
    
    /// Save a requirement to file.
    /// 
    /// The `path` is the path to the directory containing the requirement,
    /// the filename is determined by the `hrid`
    fn save(&self, path: &Path) -> io::Result<()> {
        let requirement = Requirement::from(self.clone());
        let filepath = path.join(&requirement.hrid);
        requirement.to_file(&filepath)
    }
}
